...
- replace plugin `eslint-plugin-jsx-a11y` with `eslint-plugin-jsx-a11y-x`
- replace dependency `@eslint/compat` with `@eslint/config-helpers`
- `parseGitIgnore` now can accept array of `.gitignore` files and always return array.
